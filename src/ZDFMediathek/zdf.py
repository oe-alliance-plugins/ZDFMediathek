...
class ZDFMediathek(Screen):
	...
	def HauptMenu(self, index="0"):
		menu = [
			("LISTE", "ZDF Mediathek - Start", API_URL + "start-page", "", PLUGINPATH + "/img/" + "home.png", ""),
			("LISTE", "ZDF Info", API_URL + "document/zdf-info-100", "", PLUGINPATH + "/img/" + "home.png", ""),
			("LISTE", "ZDF tivi", API_URL + "document/zdftivi-fuer-kinder-100", "", PLUGINPATH + "/img/" + "home.png", ""),
			("LISTE", "Rubriken", API_URL + "categories-overview", "", PLUGINPATH + "/img/" + "endecken.png", ""),
			("LISTE", "Sendung  A - Z", API_URL + "document/sendungen-100", "", PLUGINPATH + "/img/" + "az.png", ""),
			("BROADCAST", "Sendung  Verpasst", "", "", PLUGINPATH + "/img/" + "programm.png", ""),
	...
	def DL_finished(self, s=""):
		...
		self["DownloadLabel"].hide()
		self.DL_File = None
		self.session.open(MessageBox, f"Download erfolgreich {s}", MessageBox.TYPE_INFO, timeout=5)
	...
	def DL_failed(self, error):
		...
		if path.exists(self.DL_File):
			unlink(self.DL_File)
		self.DL_File = None
		self.session.open(MessageBox, f"Download-Fehler {error}", MessageBox.TYPE_INFO)
	...
	def search(self, search):
		if search:
			url = API_URL + f"search?q={quote_plus(search)}"
	...
	def Broadcast(self, index="0"):
		...
		for i in range(0, 7):
			...
			url = API_URL + "broadcast-missed/{}".format((now + timedelta(-i)).strftime("%Y-%m-%d"))
	...
	def teaserContent(self, js):
		...
		if js.get("seasonNumber"):
			title = "{} - S{}E{}{}".format(js.get("headline"), js.get("seasonNumber", "0"), js.get("episodeNumber", "0"), (" - " + js.get("titel") if "Episode" not in js.get("titel") else ""))
		else:
			title = js.get("titel")
		img = ImageUrl(js)
		plot = js.get("channel", "")
